...
<center><br><br><br><br>
  <h1>History</h1>
  <div> 
		<form method="post" action="donorHistory.php">
	    Search: <select name="status"  >
                <option class="combo" value="">Please select</option>
			    <option class="combo" value="Pending">Pending</option>
				<option class="combo" value="Approved">Approved</option>
	            <option class="combo" value="Completed">Completed</option>
	            <option class="combo" value="Cancelled">Cancelled</option>
			    </select>
        ...
        <?php
			...
			if (isset($_POST['searchBtn']))
			{
				...
				$sql = "SELECT appointment.AppointmentID, appointment.Name, appointment.Email, appointment.Phone, appointment.Date, appointment.Time, appointment.Status, appointment.DonorID 
						...
						WHERE Status LIKE '%".$status."%' and donor.DonorID = '{$_SESSION["id"]}'" or die(mysqli_error());
			}
			else
			{
		       include ("newconn.php"); 
				...
				$sql = "SELECT appointment.AppointmentID, appointment.Name, appointment.Email, appointment.Phone, appointment.Date, appointment.Time, appointment.Status, appointment.DonorID 
						FROM appointment INNER JOIN donor on appointment.DonorID = donor.DonorID
						WHERE donor.DonorID = '{$_SESSION["id"]}'" or die(mysqli_error());
            }
